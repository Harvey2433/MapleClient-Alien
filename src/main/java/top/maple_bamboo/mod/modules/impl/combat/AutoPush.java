...
import top.maple_bamboo.mod.modules.impl.exploit.Blink;
import top.maple_bamboo.mod.modules.impl.player.PacketMine;
...
public class AutoPush extends Module {
    ...
    public static void pistonFacing(Direction i) {
        if (i == Direction.EAST) {
            MapleClientMain.ROTATION.snapAt(-90.0f, 5.0f);
        } else if (i == Direction.WEST) {
            MapleClientMain.ROTATION.snapAt(90.0f, 5.0f);
        } else if (i == Direction.NORTH) {
            MapleClientMain.ROTATION.snapAt(180.0f, 5.0f);
        } else if (i == Direction.SOUTH) {
            MapleClientMain.ROTATION.snapAt(0.0f, 5.0f);
        }
    }
            ...
            if (BlockUtil.clientCanPlace(piston)) {
                ...
                if (BlockUtil.getPlaceSide(piston, placeRange.getValue()) != null) {
                    ...
                    if (canPower) {
                        ...
                        if (side != null) {
                            if (rotate.getValue()) MapleClientMain.ROTATION.lookAt(piston.offset(side), side.getOpposite());
                            ...
                            if (rotate.getValue() && yawDeceive.getValue()) MapleClientMain.ROTATION.lookAt(piston.offset(side), side.getOpposite());
                            if (rotate.getValue() && AntiCheat.INSTANCE.snapBack.getValue()) {
                                MapleClientMain.ROTATION.snapBack();
                            }
                            for (Direction i : Direction.values()) {
                                if (getBlock(piston.offset(i)) == getBlockType()) {
                                    if (mine.getValue()) {
...
/*                        if (mine.getValue()) {
    ...
    private boolean isTrueFacing(BlockPos pos, Direction facing) {
        ...
        float[] rotation = MapleClientMain.ROTATION.getRotation(directionVec);
    ...
    private Boolean canPush(PlayerEntity player) {
        ...
        if (!allowWeb.getValue() && MapleClientMain.PLAYER.isInWeb(player)) return false;
        float[] offset = new float[]{-0.25f, 0f, 0.25f};
        ...
        return progress > surroundCheck.getValue() - 1 || MapleClientMain.HOLE.isHard(new BlockPosX(player.getX(), player.getY() + 0.5, player.getZ()));
    }
