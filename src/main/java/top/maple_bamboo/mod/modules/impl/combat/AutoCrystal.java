...
import top.maple_bamboo.mod.modules.impl.render.ExplosionSpawn;
...
import static top.maple_bamboo.api.utils.world.BlockUtil.getBlock;
import static top.maple_bamboo.api.utils.world.BlockUtil.hasCrystal;
...
public class AutoCrystal extends Module {
    ...
    public AutoCrystal() {
        ...
        setChinese("自动水晶");
        INSTANCE = this;
        MapleClientMain.EVENT_BUS.subscribe(new CrystalRender());
    }
    ...
    private void doBreak(BlockPos pos) {
        ...
        for (EndCrystalEntity entity : BlockUtil.getEndCrystals(new Box(pos.getX(), pos.getY(), pos.getZ(), pos.getX() + 1, pos.getY() + 2, pos.getZ() + 1))) {
            ...
            if (crystalPos != null && displayTarget != null && lastDamage >= getDamage(displayTarget) && afterBreak.getValue()) {
                if (!yawStep.getValue() || !checkFov.getValue() || MapleClientMain.ROTATION.inFov(entity.getPos(), fov.getValueFloat())) {
                    doPlace(crystalPos);
                }
            }
