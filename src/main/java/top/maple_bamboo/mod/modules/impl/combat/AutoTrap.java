...
import top.maple_bamboo.mod.modules.settings.Placement;
...
public class AutoTrap
    ...
    private void trapTarget(PlayerEntity target) {
        if (onlyHole.getValue() && !MapleClientMain.HOLE.isHole(EntityUtil.getEntityPos(target))) return;
    ...
    private void doTrap(BlockPos pos) {
        ...
        if (antiStep.getValue() && (MapleClientMain.BREAK.isMining(pos.up(2)) || !onlyBreak.getValue())) {
        ...
        if (chestUp.getValue()) {
            for (Direction i : Direction.values()) {
                ...
                if (!onlyBreaking.getValue() || MapleClientMain.BREAK.isMining(pos.up(2))) {
                    tryPlaceObsidian(offsetPos);
    ...
    public BlockPos getHelper(BlockPos pos) {
        ...
        for (Direction i : Direction.values()) {
            if (checkMine.getValue() && MapleClientMain.BREAK.isMining(pos.offset(i))) continue;
    ...
    private void tryPlaceBlock(BlockPos pos, boolean anchor, boolean sand, boolean web) {
        ...
        if (MapleClientMain.BREAK.isMining(pos)) return;
