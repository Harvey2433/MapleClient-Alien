...
public class AutoWeb extends Module {
                ...
                for (float z : new float[]{0, offset.getValueFloat(), -offset.getValueFloat()}) {
                    for (float y : new float[]{0, 1, -1}) {
                        ...
                        if (!list.contains(pos)) {
                            list.add(pos);
                            if (isTargetHere(pos, player) && mc.world.getBlockState(pos).getBlock() == Blocks.COBWEB && !MapleClientMain.BREAK.isMining(pos)) {
                                webs++;
                            }
                        }
                    }
    ...
    private boolean placeWeb(BlockPos pos) {
        ...
        if (detectMining.getValue() && (MapleClientMain.BREAK.isMining(pos))) return false;
