package top.maple_bamboo.mod.gui.clickgui.components.impl;

import top.maple_bamboo.MapleClientMain;
import top.maple_bamboo.api.utils.math.Animation;
import top.maple_bamboo.mod.modules.impl.client.ClickGui;
import top.maple_bamboo.mod.modules.settings.impl.EnumSetting;
import top.maple_bamboo.core.impl.GuiManager;
import top.maple_bamboo.api.utils.render.Render2DUtil;
import top.maple_bamboo.api.utils.render.TextUtil;
import top.maple_bamboo.mod.gui.clickgui.components.Component;
import top.maple_bamboo.mod.gui.clickgui.ClickGuiScreen;
import top.maple_bamboo.mod.gui.clickgui.tabs.ClickGuiTab;
...
public class EnumComponent extends Component {
	...
	public boolean draw(int offset, DrawContext drawContext, float partialTicks, Color color, boolean back) {
		...
		if (ClickGui.INSTANCE.mainEnd.booleanValue) {
			Render2DUtil.drawRectHorizontal(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : MapleClientMain.GUI.getColor(), ClickGui.INSTANCE.mainEnd.getValue());
		} else {
			Render2DUtil.drawRect(matrixStack, (float) x + 1, (float) y + 1, (float) width - 2, (float) defaultHeight - (ClickGui.INSTANCE.maxFill.getValue() ? 0 : 1), hover ? ClickGui.INSTANCE.mainHover.getValue() : MapleClientMain.GUI.getColor());
		}
