...
import top.maple_bamboo.api.events.impl.*;
import top.maple_bamboo.api.events.impl.*;
...
import top.maple_bamboo.mod.modules.impl.movement.MoveFix;
...
public class RotationManager implements Wrapper {
    ...
    public void snapBack() {
        mc.getNetworkHandler().sendPacket(new PlayerMoveC2SPacket.Full(mc.player.getX(), mc.player.getY(), mc.player.getZ(), MapleClientMain.ROTATION.rotationYaw, MapleClientMain.ROTATION.rotationPitch, mc.player.isOnGround()));
    }
    ...
    public void snapAt(Vec3d directionVec) {
        ...
        if (AntiCheat.INSTANCE.noSpamRotation.getValue()) {
            if (MathHelper.angleBetween(angle[0], MapleClientMain.ROTATION.lastYaw) < AntiCheat.INSTANCE.fov.getValueFloat() && Math.abs(angle[1] - MapleClientMain.ROTATION.lastPitch) < AntiCheat.INSTANCE.fov.getValueFloat()) {
                return;
            }
        }
    ...
    public void update(MovementPacketsEvent event) {
        if (MoveFix.INSTANCE.isOn()) {
            ...
            MapleClientMain.EVENT_BUS.post(event1);
    ...
    private void updateNext() {
        ...
        MapleClientMain.EVENT_BUS.post(rotateEvent);
    ...
    public void onLastRotation(RotateEvent event) {
        ...
        MapleClientMain.EVENT_BUS.post(lookAtEvent);
